...
module.exports = {
    ...
    async store({ flash, res, conf }){
        ...
        let d = resolve(process.cwd(), conf.formFileDir, h);
        await fs.mkdir(d, { recursive: true });

        for(let f of flash.files)
            await fs.rename(resolve(process.cwd(), f.tempFilePath), d + '/' + f.name);
    ...
    async confirm({ conf, body, res }){
        ...
        let d = resolve(process.cwd(), conf.formFileDir, body.hash || 'none');
