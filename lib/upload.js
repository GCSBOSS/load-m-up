...
const TOO_BIG_MSG = 'Exceeded max file size allowed of %d bytes';
...
module.exports = {
    ...
    async store({ req, res, conf }){
        ...
        valid(req.files && req.files.file, NO_DATA_MSG);
        ...
        let h = createHash('sha256').update(new Date() + f.name).digest('hex');
    ...
    async confirm({ conf, body, res }){
        ...
        let token = process.env[conf.tokenEnvVar] || 'none';
